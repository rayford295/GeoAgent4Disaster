...
try:
    ...
    GOOGLE_API_KEY = "YOUR_API_KEY_HERE"  # 请在此填入你的 Key
...
class DamageRecognitionAgent:
    ...
    def _step_2_diagnose(self, image_patch: Image.Image, category: str) -> Dict[str, Any]:
        ...
        # 更新：融入了针对飓风街景的详细分类标准 (Mild/Moderate/Severe)
        prompt = f"""
        You are a disaster assessment expert analyzing a street-view image patch of a {category}.
        
        Perform the following reasoning steps:
        
        Step 1: Analyze visual features (e.g., tree integrity, debris amount, water levels, structural condition).
        Step 2: Determine the specific damage type (e.g., Structural Failure, Flooding, Tree Fall, Debris).
        
        Step 3: Assess the damage level based on the following strict criteria:
        ...
        {{
            ...
            "Reasoning": "Brief explanation citing the visual evidence (e.g., 'Found visible pools of water around tree base')"
        }}
        """

        try:
            # 街景图像细节丰富，使用 1.5 Pro 进行推理效果最好
            response = self.client.models.generate_content(
            ...
            return {"Damage_Type": "Unknown", "Level": "Unknown", "Confidence": 0.0, "Reasoning": "Error"}
...
# ==========================================
# Part 4: 主程序执行 (Main Execution)
# ==========================================

def main():
    # 1. 初始化 Agent
    agent = DamageRecognitionAgent(api_key=os.environ["GEMINI_API_KEY"])

    # 2. 设置图片路径 (请确保 Colab 文件区有这张图片)
    image_path = "hurricane_street_view.jpg" 
    
    # 下载一张测试图 (可选)
    if not os.path.exists(image_path):
        print("Downloading sample street view image...")
        !wget -O hurricane_street_view.jpg "https://upload.wikimedia.org/wikipedia/commons/e/e0/Hurricane_Ike_damage_in_Galveston.jpg"

    # 3. 运行 Agent
    print(f"Processing {image_path}...")
    evidence_df, processed_img, masks = agent.execute(image_path)

    # 4. 显示结构化输出 (Structured Output)
    if not evidence_df.empty:
        print("\n=== Structured Damage Description Table ===")
        display(evidence_df) 
        
        # 5. 可视化结果
        print("\n=== Visualized Evidence ===")
        final_vis = visualize_results(processed_img, masks, evidence_df)
        display(final_vis)
    else:
        print("No damage detected or image load failed.")
